...
satellite/models/sayso.yaml  (LiveKit prod scale; Piper default)
  ...
  -> [optional] verified hard-negative overlay (train negatives only)
...
Optional **verified hard-negative overlay** may be appended after augment via
`wake_livekit_run.py run` with `--hard-negative-features` and
`--hard-negative-provenance`. The runner verifies provenance (sha256, dtype,
shape, selected count) and atomically appends only to
`<output>/<model>/negative_features_train.npy`. It does not modify positives,
test negatives, background test features, `validation_set_features.npy`, or the
ACAV source bank.

Overlay rows are **(N, 16, 96) embeddings**, never wavs:

- ACAV bank via `wake_hard_negative_mine.py` (top-K, diversity gap).
- Corpus false-positive / near-miss windows (AMI train meetings, VOiCES train
  rooms, labelled Snowball FPs) via `wake_audio_hard_negative_mine.py` — same
  2 s / 160 ms / +10 dB path as replay. Score floor 0.25. Do not copy those
  wavs into `negative_train` or `custom_negative_phrases`.
